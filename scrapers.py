...
def scrape_play_store(limit=150):
    """Scrape reviews from Google Play Store for Blinkit."""
    reviews_list = []
    active_keywords = db_client.fetch_keywords()
    
    try:
        from google_play_scraper import reviews as gp_reviews, Sort
        print(f"[Play Store Scraper] Scraping {limit} reviews for {PLAY_STORE_APP_ID}...")
        
        result, _ = gp_reviews(
            PLAY_STORE_APP_ID,
            lang='en',
            country='in',  # India store focus
            sort=Sort.NEWEST,
            count=limit
        )
        ...
        print(f"[Play Store Scraper] Ingested {len(reviews_list)} matching reviews.")
    except Exception as e:
        print(f"[Play Store Scraper] Scraper failed (using SQLite fallback simulation if offline): {e}")
        # Insert some seed records if offline or query fails
        reviews_list = get_simulated_scraped_data("Google Play")
        
    return reviews_list

def scrape_app_store(limit=100):
    """Scrape reviews from Apple App Store RSS feed for Blinkit (India)."""
    reviews_list = []
    active_keywords = db_client.fetch_keywords()
    
    # Apple iTunes RSS feed format for India customer reviews
    url = f"https://itunes.apple.com/in/rss/customerreviews/id={APP_STORE_APP_ID}/sortBy=mostRecent/json"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    
    try:
        print(f"[App Store Scraper] Fetching reviews from RSS feed for ID {APP_STORE_APP_ID}...")
        ...
        if response.status_code == 200:
            ...
            print(f"[App Store Scraper] Ingested {len(reviews_list)} matching reviews.")
        else:
            print(f"[App Store Scraper] Apple RSS feed returned HTTP {response.status_code}")
            reviews_list = get_simulated_scraped_data("App Store")
    except Exception as e:
        print(f"[App Store Scraper] Scraper failed: {e}")
        reviews_list = get_simulated_scraped_data("App Store")
        
    return reviews_list

def scrape_reddit():
    """Scrape comments from curated Reddit threads using PRAW (feature-flagged)."""
    reviews_list = []
    active_keywords = db_client.fetch_keywords()
    
    if not (REDDIT_CLIENT_ID and REDDIT_CLIENT_SECRET):
        print("[Reddit Scraper] OAuth credentials missing. Running in stub mode.")
        return get_simulated_scraped_data("Reddit")
    ...
    try:
        ...
        for url in REDDIT_SEED_URLS:
            ...
            submission.comments.replace_more(limit=0)  # Top-level comments only for simplicity
        ...
        print(f"[Reddit Scraper] Ingested {len(reviews_list)} matching comments.")
    except Exception as e:
        print(f"[Reddit Scraper] PRAW extraction failed: {e}")
        reviews_list = get_simulated_scraped_data("Reddit")
        
    return reviews_list

def get_simulated_scraped_data(source):
    """Generate high-quality simulated Blinkit review data for category discovery exploration fallback."""
    print(f"[Scraper Fallback] Generating offline simulated feedback for {source}...")
    import random
    from datetime import datetime, timedelta
    
    # Blinkit category exploration pain points & positive behaviors
    TEMPLATES = [
    ...
    # Generate 15-20 reviews
    for i in range(random.randint(15, 20)):
        text = random.choice(TEMPLATES)
        if random.random() > 0.5:
            text += f" (Test Review ID: {int(datetime.now().timestamp())}_{i})"
            
        review_id_raw = f"simulated_{source.lower()}_{i}_{int(datetime.now().timestamp())}"
